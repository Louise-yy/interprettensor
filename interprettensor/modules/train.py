    ...
    def optimize(self):
        ...
        with tf.name_scope('train'):
            if self.optimizer == 'adam':
                self.train = tf.compat.v1.train.AdamOptimizer(self.learning_rate).minimize(self.cost)

            elif self.optimizer == 'rmsprop':
                self.train = tf.compat.v1.train.AdamOptimizer(self.learning_rate).minimize(self.cost)

            elif self.optimizer == 'grad_descent':
                self.train = tf.compat.v1.train.GradientDescentOptimizer(self.learning_rate).minimize(self.cost)
            elif self.optimizer == 'adagrad':
                self.train = tf.compat.v1.train.AdagradOptimizer(self.learning_rate).minimize(self.cost)

            elif self.optimizer == 'adadelta':
                self.train = tf.compat.v1.train.AdadeltaOptimizer(self.learning_rate).minimize(self.cost)
            else:
