...
class Linear(Module):
    ...
    def __init__(self, output_dim, batch_size=None, input_dim=None, act='linear', batch_norm=False,
                 batch_norm_params={'momentum': 0.9, 'epsilon': 1e-5, 'training': False, 'name': 'bn'},
                 keep_prob=tf.constant(1.0), weights_init=tf.compat.v1.truncated_normal_initializer(stddev=0.01),
                 bias_init=tf.constant_initializer(0.0), name="linear"):
        self.name = name
    ...
    def _simple_lrp(self, R):
        ...
        return self.input_tensor * tf.reduce_sum(
            tf.expand_dims(self.weights, 0) * tf.expand_dims(self.R / (self.activations + 1e-3), 1), -1)
    ...
    def _epsilon_lrp(self, R, epsilon):
        ...
        return self.input_tensor * tf.reduce_sum(
            tf.expand_dims(self.weights, 0) * tf.expand_dims(self.R / (self.activations + epsilon), 1), -1)
    ...
    def _alphabeta_lrp(self, R, alpha):
        ...
        if not alpha == 0:
            ...
            term2 = tf.expand_dims(
                tf.expand_dims(tf.where(tf.greater(self.biases, 0), self.biases, tf.zeros_like(self.biases)), 0), 0)
        ...
        if not beta == 0:
            ...
            term2 = tf.expand_dims(
                tf.expand_dims(tf.where(tf.less(self.biases, 0), self.biases, tf.zeros_like(self.biases)), 0), 0)
