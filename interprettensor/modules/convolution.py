...
class Convolution(Module):
    ...
    def __init__(self, output_depth, batch_size=None, input_dim=None, input_depth=None, kernel_size=5, stride_size=2,
                 act='linear', batch_norm=False,
                 batch_norm_params=None, keep_prob=1.0, pad='SAME',
                 weights_init=tf.compat.v1.truncated_normal_initializer(stddev=0.01),
                 bias_init=tf.constant_initializer(0.0), name="conv2d"):
        if batch_norm_params is None:
            batch_norm_params = {'momentum': 0.9, 'epsilon': 1e-5, 'training': False, 'name': 'bn'}
        self.name = name
    ...
    def _simple_lrp(self, R):
        ...
        result = image_patches * tf.reduce_sum(
            tf.expand_dims(self.weights, 0) * tf.reshape((self.R / (self.activations + 1e-8)),
                                                         [self.in_N, self.Hout, self.Wout, 1, 1, 1, self.output_depth]),
            -1)
    ...
    def _epsilon_lrp(self, R, epsilon):
        ...
        image_patches = self.extract_patches()
        result = image_patches * tf.reduce_sum(
            tf.expand_dims(self.weights, 0) * tf.reshape((self.R / (self.activations + epsilon)),
                                                         [self.in_N, self.Hout, self.Wout, 1, 1, 1, self.output_depth]),
            -1)
    ...
    def _ww_lrp(self, R):
        ...
        result = tf.reduce_sum(Z * tf.reshape((self.R / (tf.reduce_sum(Z, [1, 2, 3]) + 1e-16)),
                                              [self.in_N, self.Hout, self.Wout, 1, 1, 1, self.output_depth]), -1)
        return self.restitch_image(result)
    ...
    def _flat_lrp(self, R):
        ...
        self.check_shape(R)
        Z = tf.ones(
            [self.in_N, self.Hout, self.Wout, self.kernel_size, self.kernel_size, self.in_depth, self.output_depth])
    ...
    def extract_patches(self):
        image_patches = tf.extract_image_patches(self.input_tensor, ksizes=[1, self.kernel_size, self.kernel_size, 1],
                                                 strides=[1, self.stride_size, self.stride_size, 1], rates=[1, 1, 1, 1],
                                                 padding=self.pad)
        return tf.reshape(image_patches,
                          [self.in_N, self.Hout, self.Wout, self.kernel_size, self.kernel_size, self.in_depth])
    ...
    def compute_result(self, Z, epsilon=1e-12):
        return tf.reduce_sum(Z * tf.reshape((self.R / (tf.reduce_sum(Z, [3, 4, 5]) + epsilon)),
                                            [self.in_N, self.Hout, self.Wout, 1, 1, 1, self.output_depth]), -1)

    def restitch_image(self, result):
        return self.patches_to_images(result, self.in_N, self.in_h, self.in_w, self.in_depth, self.Hout, self.Wout,
                                      self.kernel_size, self.kernel_size, self.stride_size, self.stride_size)

    def clean(self):
        self.activations = None
        self.R = None

    def patches_to_images(self, grad, batch_size, rows_in, cols_in, channels, rows_out, cols_out, ksize_r, ksize_c,
                          stride_h, stride_r):
        rate_r = 1
        rate_c = 1
        padding = self.pad
    ...
    def __simple_lrp(self, R):
        ...
        if self.pad == 'SAME':
            ...
            p_left = pc / 2
            p_right = pc - (pc / 2)
            self.pad_input_tensor = tf.pad(self.input_tensor, [[0, 0], [p_top, p_bottom], [p_left, p_right], [0, 0]],
                                           "CONSTANT")
        elif self.pad == 'VALID':
    ...
    def __epsilon_lrp(self, R, epsilon):
        ...
        if self.pad == 'SAME':
            pr = (Hout - 1) * hstride + hf - in_h
            pc = (Wout - 1) * wstride + wf - in_w
            self.pad_input_tensor = tf.pad(self.input_tensor,
        ...
        for i in xrange(Hout):
            for j in xrange(Wout):
                ...
                if not alpha == 0:
                    Zp = tf.where(tf.greater(Z, 0), Z, tf.zeros_like(Z))
                    t2 = tf.expand_dims(
                        tf.expand_dims(tf.where(tf.greater(self.biases, 0), self.biases, tf.zeros_like(self.biases)),
                                       0), 0)
                ...
                if not beta == 0:
                    Zn = tf.where(tf.less(Z, 0), Z, tf.zeros_like(Z))
                    t2 = tf.expand_dims(
                        tf.expand_dims(tf.where(tf.less(self.biases, 0), self.biases, tf.zeros_like(self.biases)), 0),
                        0)
