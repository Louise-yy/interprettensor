...
# input dict creation as per tensorflow source code 根据张量流源代码创建输入字典
...
def train():
    ...
    mnist = input_data.load_data(FLAGS.data_dir)

    with tf.compat.v1.Session() as sess:
        # Input placeholders
        with tf.name_scope('input'):
            x = tf.compat.v1.placeholder(tf.float32, [FLAGS.batch_size, 784], name='x-input')
            y_ = tf.compat.v1.placeholder(tf.float32, [FLAGS.batch_size, 10], name='y-input')
        ...
        with tf.compat.v1.variable_scope('relevance'):
            if FLAGS.relevance:
        ...
        test_writer = tf.compat.v1.summary.FileWriter(FLAGS.summaries_dir + '/test')
        ...
        uninit_vars = set(tf.compat.v1.global_variables()) - set(tf.compat.v1.trainable_variables())
        tf.compat.v1.variables_initializer(uninit_vars).run()

        # iterate over train and test data
        for i in range(FLAGS.max_steps):
            if i % FLAGS.test_every == 0:
                # pdb.set_trace()
                d = feed_dict(mnist, False)  # 生成测试数据的字典
                test_inp = {x: d[0], y_: d[1], keep_prob: d[2]}
                summary, acc, relevance_test, op, rel_layer = sess.run([merged, accuracy, LRP, y, relevance_layerwise],
                                                                       feed_dict=test_inp)
                ...
                d = feed_dict(mnist, True)  # 生成训练数据的字典
                inp = {x: d[0], y_: d[1], keep_prob: d[2]}
                summary, _, relevance_train, op, rel_layer = sess.run(
                    [merged, trainer.train, LRP, y, relevance_layerwise], feed_dict=inp)
